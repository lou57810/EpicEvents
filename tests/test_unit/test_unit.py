...
class TestUserSignIn:
    ...
    def test_user_sign_in(self, instance, monkeypatch, capsys):
        ...
        monkeypatch.setattr('maskpass.askpass',
                            lambda prompt, mask: 'password123')
...
"""class TestUserController:
    ...
    def test_check_email_with_correct_email(
            self, instance, monkeypatch, mocker, capsys):
        session_app = EngineController()
        # Mock user input
        # monkeypatch.setattr('view.start_menu_view.StartMenuView.\
            user_sign_in', lambda _: ('test@example.com', 'password123'))
        monkeypatch.setattr('view.start_menu_view.StartMenuView.\
            user_sign_in', lambda _: ('test@example.com', 'password123'))
        ...
        monkeypatch.setattr('controller.user_controller.session.query(User).\
            filter_by', lambda **kwargs: user_row_mock)
        ...
        check_password_mock.assert_called_once_with(
            'password123', user_row_mock)"""
